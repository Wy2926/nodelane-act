...
export const pages: Record<'zh' | 'en', Record<string, PageContent>> = {
  zh: {
    codex: {
      ...
      sections: [
        { heading: '通过 Smithery 市场安装', body: ['NodeLane Act 0.1.0 已发布到 Smithery 与官方 MCP Registry。运行下面对应客户端的命令，再加载同版本浏览器扩展。Smithery 可能要求账号登录；不使用市场账号时可用官网 TGZ 或 ZIP。','Smithery 对 Claude Code 默认安装到当前项目。npm 同名短包目前尚未发布，请优先使用这里的市场命令。'], code: 'npx -y smithery@latest mcp add q921381971/nodelane-act --client codex' },
    ...
    'claude-code': {
      ...
      sections: [
        { heading: '通过 Smithery 市场安装', body: ['NodeLane Act 0.1.0 已发布到 Smithery 与官方 MCP Registry。运行下面对应客户端的命令，再加载同版本浏览器扩展。Smithery 可能要求账号登录；不使用市场账号时可用官网 TGZ 或 ZIP。','Smithery 对 Claude Code 默认安装到当前项目。npm 同名短包目前尚未发布，请优先使用这里的市场命令。'], code: 'npx -y smithery@latest mcp add q921381971/nodelane-act --client claude-code' },
    ...
    cursor: {
      ...
      sections: [
        { heading: '通过 Smithery 市场安装', body: ['NodeLane Act 0.1.0 已发布到 Smithery 与官方 MCP Registry。运行下面对应客户端的命令，再加载同版本浏览器扩展。Smithery 可能要求账号登录；不使用市场账号时可用官网 TGZ 或 ZIP。','Smithery 对 Claude Code 默认安装到当前项目。npm 同名短包目前尚未发布，请优先使用这里的市场命令。'], code: 'npx -y smithery@latest mcp add q921381971/nodelane-act --client cursor' },
  ...
  en: {
    codex: {
      ...
      sections: [
        { heading: 'Install from Smithery', body: ['NodeLane Act 0.1.0 is published on Smithery and the official MCP Registry. Run the command below for this client, then load the companion browser extension. Smithery may require account login; the direct website package and ZIP remain available as alternatives.','Smithery installs Claude Code in the current project by default. The npm short package name is not published.'], code: 'npx -y smithery@latest mcp add q921381971/nodelane-act --client codex' },
